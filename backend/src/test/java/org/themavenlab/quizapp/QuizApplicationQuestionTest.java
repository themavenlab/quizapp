...
public class QuizApplicationQuestionTest {
	...
	@Test
	void questionDeserializationTest() throws IOException {
		String expected = """
				{
				  "id": 1,
				  "text": "How filters are used in Spring Web?",
				  "category": "Spring Framework"
				}
				""";

		assertThat(json.parse(expected))
				.isEqualTo(new Question(1L, "How filters are used in Spring Web?", "Spring Framework"));

		assertThat(json.parseObject(expected).id()).isEqualTo(1L);
		assertThat(json.parseObject(expected).text()).isEqualTo("How filters are used in Spring Web?");
		assertThat(json.parseObject(expected).category()).isEqualTo("Spring Framework");
	}
}
